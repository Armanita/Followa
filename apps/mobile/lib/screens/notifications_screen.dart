...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final response =
          await AuthService.instance.get('/notifications?limit=100');
  ...
  Future<void> _readAll() async {
    try {
      await AuthService.instance.post('/notifications/read-all');
      await _load();
    } catch (error) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('اعلان‌ها'), actions: [
        TextButton(onPressed: _readAll, child: const Text('خواندن همه'))
      ]),
          ...
          child: ListView(
              ...
              children: [
                if (_loading)
                  ...
                  const PremiumPanel(child: EmptyState(title: 'اعلانی ندارید'))
                else
                  ..._items.map((raw) {
