...
class _CustomersScreenState extends State<CustomersScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (_search.text.trim().isNotEmpty) {
        params.add('search=${Uri.encodeComponent(_search.text.trim())}');
      }
      if (_showArchived) params.add('active=all');
      final response =
          await AuthService.instance.get('/customers?${params.join('&')}');
  ...
  Future<void> _toggleArchive(Map<String, dynamic> customer) async {
    ...
    final ok = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
              ...
              content: Text(active
                  ? 'پرونده‌های قبلی حفظ می‌شوند اما مشتری تا زمان بازیابی برای پرونده جدید قابل انتخاب نیست.'
                  : 'مشتری دوباره برای اتصال به پرونده جدید فعال می‌شود.'),
              actions: [
                TextButton(
                ...
                FilledButton(
                    ...
                    child: Text(active ? 'بایگانی' : 'بازیابی'))
              ],
            ));
    if (ok != true) return;
    try {
      await AuthService.instance.post(
          '/customers/${customer['id']}/${active ? 'archive' : 'restore'}');
      await _load();
    } catch (error) {
  ...
  Future<void> _form([Map<String, dynamic>? customer]) async {
    final type =
        ValueNotifier<String>(customer?['type']?.toString() ?? 'INDIVIDUAL');
    final name =
    ...
    final nationalId =
        TextEditingController(text: customer?['nationalId']?.toString() ?? '');
    final economicCode = TextEditingController(
        text: customer?['economicCode']?.toString() ?? '');
    final email =
            ...
            builder: (ctx, setD) => AlertDialog(
                      ...
                      child: SingleChildScrollView(
                        ...
                        DropdownButtonFormField<String>(
                            ...
                            items: const [
                              DropdownMenuItem(
                                  value: 'INDIVIDUAL',
                                  child: Text('شخص حقیقی')),
                              DropdownMenuItem(
                                  value: 'LEGAL', child: Text('شخص حقوقی'))
                            ],
                            onChanged: (v) {
                        ...
                        TextField(
                            ...
                            decoration: const InputDecoration(
                                labelText: 'کد/شناسه ملی')),
                        const SizedBox(height: 9),
                        TextField(
                            controller: economicCode,
                            textDirection: TextDirection.ltr,
                            decoration:
                                const InputDecoration(labelText: 'کد اقتصادی')),
                        const SizedBox(height: 9),
                        TextField(
                            controller: email,
                            textDirection: TextDirection.ltr,
                  ...
                  actions: [
                    TextButton(
                        ...
                        child: const Text('انصراف')),
                    FilledButton(
                        onPressed: busy || name.text.trim().length < 2
                            ? null
                            : () async {
                                setD(() => busy = true);
                                final payload = {
                                  ...
                                  'economicCode':
                                      economicCode.text.trim().isEmpty
                                          ? null
                                          : economicCode.text.trim(),
                                ...
                                try {
                                  if (customer == null) {
                                    ...
                                    await AuthService.instance.patch(
                      ...
                      Flexible(
                          child: items.isEmpty
                              ? const Padding(
                                  ...
                                  child: Text('پرونده‌ای ثبت نشده است.'))
                              : ListView.separated(
                                  shrinkWrap: true,
                                  ...
                                  itemBuilder: (_, i) {
                                    ...
                                    return ListTile(
                                        ...
                                        subtitle: Text(
                                            '#${Fa.num(c['number'])} · ${statusLabels[c['status']] ?? c['status']}'),
                                        onTap: () {
                                          Navigator.pop(ctx);
                                          Navigator.push(
                                              context,
          ...
          child: ListView(
              ...
              children: [
                PremiumPanel(
                  ...
                  TextField(
                      ...
                      decoration: const InputDecoration(
                          ...
                          hintText: 'نام، موبایل یا شناسه…')),
                  CheckboxListTile(
                      contentPadding: EdgeInsets.zero,
                      value: _showArchived,
                      title: const Text('نمایش بایگانی‌شده‌ها',
                ...
                if (_loading)
                  ...
                  const PremiumPanel(
                      child: EmptyState(title: 'مشتری‌ای یافت نشد'))
                else
                  ..._items.map((raw) {
                    ...
                    return Padding(
                        ...
                        child: PremiumPanel(
                            accent: active ? null : FollowaColors.amber,
                            child: Column(
                                ...
                                children: [
                                  ...
                                  if ((c['mobile'] ?? '').toString().isNotEmpty)
                                    Padding(
                                        padding: const EdgeInsets.only(top: 8),
                                        child: Text(c['mobile'].toString(),
                                            textDirection: TextDirection.ltr,
                                            style: const TextStyle(
...
class _HistoryMetric extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
      ...
      child: Column(children: [
        Text(Fa.num(value),
            style: const TextStyle(
                color: FollowaColors.ink, fontWeight: FontWeight.w900)),
        Text(label,
            style: const TextStyle(color: FollowaColors.soft, fontSize: 9))
      ]));
}
