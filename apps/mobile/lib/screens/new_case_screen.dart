...
class _NewCaseScreenState extends State<NewCaseScreen> {
      ...
      try {
        ...
        if (mounted) {
          setState(() => _members =
                ...
                return m['isActive'] == true && m['role'] == 'EMPLOYEE';
              }).toList());
        }
      } catch (_) {}
    }
  ...
  Future<void> _quickCustomer() async {
            ...
            builder: (ctx, setD) => AlertDialog(
                  ...
                  content: Column(mainAxisSize: MainAxisSize.min, children: [
                    DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                              value: 'INDIVIDUAL', child: Text('شخص حقیقی')),
                          DropdownMenuItem(
    ...
    if (ok == true && customerName.length >= 2) {
      try {
        final response = await AuthService.instance.post('/customers', {
          'type': type,
          'name': customerName,
          if (customerMobile.isNotEmpty) 'mobile': customerMobile,
        ...
        if (mounted) {
          setState(() {
            ...
            _customers = [
              customer,
              ..._customers.where((raw) =>
                  (raw as Map<String, dynamic>)['id'] != customer['id'])
            ];
          });
        }
      } catch (error) {
  ...
  Future<void> _submit() async {
    ...
    try {
      await AuthService.instance.post('/cases', {
        ...
        if (_assignTo != null && _assignNote.text.trim().isNotEmpty)
          'assignNote': _assignNote.text.trim(),
      });
            ...
            PremiumPanel(
                child: Column(
                  ...
                  const _Title(
                      ...
                      title: 'طبقه‌بندی',
                      subtitle: 'اولویت و نوع پرونده را مشخص کنید.'),
                  const SizedBox(height: 14),
                  DropdownButtonFormField<String>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(
                            value: 'NORMAL', child: Text('معمولی')),
                ...
                child: Column(
                  ...
                  Row(children: [
                    const Expanded(
                        child: _Title(
                            ...
                            title: 'مشتری (اختیاری)',
                            subtitle:
                                'پرونده داخلی می‌تواند بدون مشتری باشد.')),
                    TextButton.icon(
                        onPressed: _quickCustomer,
                        icon: const Icon(Icons.add_rounded, size: 17),
                        label: const Text('مشتری سریع'))
                  ]),
                  ...
                  DropdownButtonFormField<String>(
                      ...
                      items: [
                        const DropdownMenuItem(
                            value: null,
                            child: Text('— بدون مشتری / پرونده داخلی —')),
              ...
              PremiumPanel(
                  child: Column(
                    ...
                    const _Title(
                        ...
                        title: 'ارجاع اولیه',
                        subtitle: 'فقط کارکنان فعال شرکت مقصد ارجاع هستند.'),
                    const SizedBox(height: 12),
                    DropdownButtonFormField<String>(
                        initialValue: _assignTo,
                        decoration:
                            const InputDecoration(labelText: 'ارجاع به'),
                        dropdownColor: FollowaColors.elevated,
                        items: [
                          const DropdownMenuItem(
                              value: null,
                              child: Text('بدون ارجاع — فقط ثبت پرونده')),
                    ...
                    if (_assignTo != null) ...[
                      const SizedBox(height: 10),
                      TextField(
                          controller: _assignNote,
                          maxLines: 2,
                          decoration:
                              const InputDecoration(labelText: 'توضیح ارجاع'))
                    ],
                  ])),
            ],
            if (_error != null) ...[
              ...
              Text(_error!,
                  style:
                      const TextStyle(color: Color(0xFFFCA5A5), fontSize: 10.5))
            ],
            const SizedBox(height: 16),
            FilledButton.icon(
                onPressed:
                    !_busy && _title.text.trim().length >= 3 ? _submit : null,
                icon: _busy
                    ? const SizedBox(
                        width: 18,
                        height: 18,
  ...
  Widget build(BuildContext context) =>
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
            ...
            child: Icon(icon, color: FollowaColors.brandSoft, size: 17)),
        ...
        Expanded(
          ...
          Text(subtitle,
              style: const TextStyle(
                  color: FollowaColors.soft, fontSize: 9, height: 1.5))
        ]))
      ]);
}
