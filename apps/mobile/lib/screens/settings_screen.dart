...
import '../widgets/messaging_settings_panel.dart';
            ...
            builder: (ctx, setD) => AlertDialog(
                  ...
                  content: Column(mainAxisSize: MainAxisSize.min, children: [
                    TextField(
                        ...
                        autofocus: true,
                        decoration:
                            const InputDecoration(labelText: 'نام نوع پرونده')),
                    ...
                    DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                              value: '#7c4dff', child: Text('بنفش')),
                          DropdownMenuItem(
                              value: '#2563eb', child: Text('آبی')),
                          DropdownMenuItem(
                              value: '#059669', child: Text('سبز')),
                          DropdownMenuItem(
                              value: '#d97706', child: Text('نارنجی')),
                          DropdownMenuItem(
                              value: '#dc2626', child: Text('قرمز')),
                        ],
    ...
    if (ok == true && value.length >= 2) {
      try {
        await AuthService.instance
            .post('/case-types', {'name': value, 'color': color});
        await _load();
      } catch (error) {
                        ...
                        decoration: BoxDecoration(
                            ...
                            border: Border.all(
                                color: FollowaColors.brandSoft
                                    .withValues(alpha: .20))),
                        child: Text(
                            _company!['name'].toString().substring(0, 1),
                  ...
                  PremiumPanel(
                      child: Column(
                        ...
                        Row(children: [
                          const Expanded(
                              child: Column(
                                ...
                                SizedBox(height: 3),
                                Text(
                                    'دسته‌بندی‌های قابل انتخاب هنگام ایجاد پرونده',
                                    style: TextStyle(
                        ...
                        Wrap(spacing: 8, runSpacing: 8, children: [
                          for (final raw
                              in (_company!['caseTypes'] as List<dynamic>? ??
                                  const []))
                            _TypeChip(type: raw as Map<String, dynamic>)
                        ]),
                      ])),
                  const SizedBox(height: 12),
                  MessagingSettingsPanel(
                      isManager: AuthService.instance.isManager),
                ],
              ])),
    );
  }
